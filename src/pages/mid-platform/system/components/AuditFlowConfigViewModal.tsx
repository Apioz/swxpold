...
import { APPROVE_MODE_OPTIONS } from '../../../../data/mockAuditFlowConfig';
...
function renderApproveMode(mode: AuditFlowConfig['approveMode']) {
  return APPROVE_MODE_OPTIONS.find((item) => item.value === mode)?.label ?? mode;
}
  ...
  return (
    ...
    >
      <section className="audit-flow-view-section">
        ...
        <div className="audit-flow-view-grid">
          ...
          <div className="audit-flow-view-row">
            ...
            <div className="audit-flow-view-cell">
              <span className="label">管理员自审:</span>
              <span className="value">{record.selfApplyAutoPass ? '自动通过' : '否'}</span>
            </div>
          </div>
          <div className="audit-flow-view-row">
            <div className="audit-flow-view-cell">
              <span className="label">叠加审批:</span>
              <span className="value">{record.stackable ? '是' : '否'}</span>
            </div>
            <div className="audit-flow-view-cell" />
          </div>
          <div className="audit-flow-view-row">
      ...
      <section className="audit-flow-view-section">
        ...
        <div className="audit-flow-view-grid">
          <div className="audit-flow-view-row">
            <div className="audit-flow-view-cell full-width">
              ...
              <span className="value">{renderApproveMode(record.approveMode)}</span>
            </div>
          </div>
        </div>
        {record.approveMode === 'manual' && (
        ...
        {record.approveMode === 'auto' && (
          <div className="audit-flow-auto-approver-panel audit-flow-view-auto-panel">
            <div className="audit-flow-auto-approver-row">
              <span className="audit-flow-auto-approver-label">审批角色</span>
              <Tag color="processing">{ORG_ADMIN_SCOPE_LABEL}（自动通过）</Tag>
            </div>
            <div className="audit-flow-auto-hint">
              提交后由{ORG_ADMIN_SCOPE_LABEL}自动审批（按规则匹配条件解析），并非无人审批。
            </div>
          </div>
        )}
      </section>
