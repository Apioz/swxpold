...
import type { InboundRecord, StockOrderItem } from '../../types/spareParts';
import {
  INBOUND_TYPES,
  PERSONS,
  WAREHOUSES,
...
import { mockInboundRecords } from '../../data/mockStockOrders';
...
export default function InboundManagement() {
  const [spareParts, { applyInbound }] = useSparePartsStore();
  const [data, setData] = useState<InboundRecord[]>(mockInboundRecords);
  ...
  const filteredData = useMemo(() => {
    return data.filter((item) => {
      ...
      if (search.warehouseId && item.warehouseId !== search.warehouseId) {
        return false;
      }
  ...
  const handleSave = async () => {
    try {
      ...
      for (const item of selectedItems) {
        ...
        if (result) stockCodes.push(result.stockCode);
      }
      ...
      const record: InboundRecord = {
        id: String(Date.now()),
        orderNo: `PO${now.format('YYYYMMDDHHmmss')}`,
        type: values.type,
  ...
  const columns: ColumnsType<InboundRecord> = [
    ...
    { title: '入库单号', dataIndex: 'orderNo', width: 160, ellipsis: true },
    { title: '入库类型', dataIndex: 'type', width: 110 },
    { title: '入库仓库', dataIndex: 'warehouseName', width: 120 },
    { title: '入库人', dataIndex: 'person', width: 100 },
    { title: '入库时间', dataIndex: 'time', width: 140 },
    {
      title: '入库数量',
      dataIndex: 'quantity',
      width: 90,
    ...
    <div className="stock-order-page">
      <div className="search-bar">
        <Form form={form} layout="inline" className="search-form">
          <Form.Item label="入库时间" name="timeRange">
          ...
          <Form.Item label="入库类型" name="type">
            <Select
              ...
              placeholder="请选择入库类型"
              style={{ width: 160 }}
              options={INBOUND_TYPES.map((t) => ({ label: t, value: t }))}
            />
          </Form.Item>
          ...
          <Form.Item label="入库人" name="person">
            <Select
              allowClear
              placeholder="请选择入库人"
              style={{ width: 140 }}
        ...
        <Form form={addForm} layout="vertical" className="spare-modal-form">
          <div className="spare-modal-section">
            <div className="spare-modal-section-head">
              <span className="spare-modal-section-title">入库信息</span>
            </div>
            <Row gutter={[16, 0]}>
              <Col span={12}>
                <Form.Item
              ...
              <Col span={24}>
                <Form.Item label="备注" name="remark">
                  <TextArea rows={2} placeholder="请输入备注" allowClear />
                </Form.Item>
              </Col>
            </Row>
          </div>
